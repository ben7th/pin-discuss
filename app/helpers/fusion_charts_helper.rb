...
@@arr_FCColors[19] = "669900" #Shade of green
  ...
  def render_chart(str_xml,chart_width,chart_height)

    chart_swf = "/javascripts/views/discuss/fusion_charts/swf/Pie3D.swf"
    ...
    chart_id = randstr
    div_id = "chart_#{chart_id}"
    div = content_tag("div","Chart",:id=>div_id)

    xml_js = ""
    if str_xml.blank?
      xml_js = %` chart_#{chart_id}.setDataURL(#{str_url.to_json}); `
    else
      xml_js = %` chart_#{chart_id}.setDataXML(#{str_xml.to_json}); `
    end
    
    %`
      #{div}
      <script type='text/javascript'>
        var chart_#{chart_id} = new FusionCharts("#{chart_swf}",#{randstr.to_json},"#{chart_width}","#{chart_height}");
        #{xml_js}
        chart_#{chart_id}.addParam("wmode","transparent")
        chart_#{chart_id}.render(#{div_id.to_json})
      </script>
    `
  end
  # Renders a chart from the swf file passed as parameter either making use of setDataURL method or 
